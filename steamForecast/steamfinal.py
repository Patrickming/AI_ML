...
df['playtime_ratio'] = df['average_playtime'] / (df['median_playtime'] + 1)  # 避免除以0
df['year'] = df['release_date'].dt.year
df['month'] = df['release_date'].dt.month
df['owners'] = df['owners'].str.split('-').str[0].astype(int)

# 将开发者和发行者的出现频率较低的类别归类为"其他"
top_developers = df['developer'].value_counts().nlargest(10).index
df['developer'] = df['developer'].where(df['developer'].isin(top_developers), 'Other')
df['developer_encoded'] = LabelEncoder().fit_transform(df['developer'])
...
# 对于SVM
svm_model = SVC(C=1.0, gamma='scale', probability=True)
svm_model.fit(X_train_scaled, y_train)

decision_tree = DecisionTreeClassifier(max_depth=8, min_samples_split=20, min_samples_leaf=5, random_state=42)
...
xgboost_model = XGBClassifier(n_estimators=100, max_depth=16, gamma=0.1, use_label_encoder=False, eval_metric='logloss', random_state=42)
xgboost_model.fit(X_train_scaled, y_train)

lightgbm_model = LGBMClassifier(n_estimators=100, max_depth=16, num_leaves=20, random_state=42)
...
def build_cnn_model(input_shape):
    model = Sequential([
        Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(input_shape, 1), padding='same'),
        MaxPooling1D(pool_size=2),
        Flatten(),
        Dense(128, activation='relu'),  # 增加神经元
        Dense(64, activation='relu'),
        Dense(1, activation='sigmoid')
    ])
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
